...
public class GridServiceContextInjectionSelfTest extends GridCommonAbstractTest {
     ...
     */
    private void doLifeCycleTest(String svcName) throws Exception {
        // Wait until the cluster-singleton deployment is visible on the remote node before invoking it
        // (deployment propagation to other nodes may lag slightly behind the synchronous deploy call).
        assertTrue("Service is not deployed: " + svcName, GridTestUtils.waitForCondition(() ->
            grid(1).services().serviceDescriptors().stream()
                .anyMatch(desc -> svcName.equals(desc.name()) && !desc.topologySnapshot().isEmpty()),
            getTestTimeout()));
