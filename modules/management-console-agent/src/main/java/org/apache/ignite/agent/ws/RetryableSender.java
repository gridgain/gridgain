...
public class RetryableSender extends GridProcessorAdapter implements Runnable {
    ...
    public RetryableSender(GridKernalContext ctx) {
        ...
        queue = new ArrayBlockingQueue<>(DEFAULT_QUEUE_CAP);
        exSrvc = Executors.newSingleThreadExecutor(new CustomizableThreadFactory("mgmt-console-sender-"));
    ...
    boolean sendInternal(String dest, List<Object> elements) throws InterruptedException {
        ...
        WebSocketManager mgr = ((IgniteManagementConsoleProcessor)ctx.managementConsole()).webSocketManager();
