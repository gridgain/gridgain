...
namespace Apache.Ignite.Core.Tests.Cache
{
    ...
    public class CacheNodeFilterTest
    {
        ...
        /// <summary>
        /// Tests that custom node filter correctly filters cluster nodes.
        /// </summary>
        [Test]
        public void TestCustomFilter()
        {
            ...
            Assert.AreEqual(2, dataNodes.Count);
            Assert.IsTrue(dataNodes.Contains(_grid2.GetCluster().GetLocalNode()));
            Assert.IsTrue(dataNodes.Contains(_grid3.GetCluster().GetLocalNode()));
        }
        ...
        public class CustomFilter : IClusterNodeFilter
        {
            ...
            public bool Invoke(IClusterNode node)
            {
                ...
                {
                    object val;
                    if (node.Attributes.TryGetValue(AttrKey2, out val))
                    {
                        if (val.Equals(AttrVal2))
                        {
                            return true;
                        }
                    }
        ...
        /// <summary>
        /// Custom node filter that throws an exception.
        /// </summary>
        public class CustomFilterWithException : IClusterNodeFilter
        {
            /// <summary>
            /// <inheritdoc cref="IClusterNodeFilter.Invoke"/>
            /// </summary>
            public bool Invoke(IClusterNode node)
            {
                throw new ApplicationException("Custom node filter exception.");
            }
        }

        /// <summary>
        /// Tests that if a custom node filter throws an exception,
        /// than it's being registered and cache is not deployed for that node.
        /// </summary>
        [Test]
        public void CustomFilterWithExceptionShouldNotHangClusterNode()
        {
            //1. Create a custom attribute filter that trows an exception
            //2. Run it
            //3. Check that remote nodes failed? cache was not deployed?

            var cacheCfg = new CacheConfiguration
            {
                Name = Guid.NewGuid().ToString(),
                CacheMode = CacheMode.Replicated,
                NodeFilter = new CustomFilterWithException()
            };

            //Explicitly register exception type (for now):
            _grid1.GetBinary().GetBinaryType(typeof(ApplicationException));

            var cache = _grid1.CreateCache<object, object>(cacheCfg);

            ICollection<IClusterNode> dataNodes = _grid1.GetCluster().ForDataNodes(cache.Name).GetNodes();

            Assert.AreEqual(0, dataNodes.Count);
        }
    }
}
