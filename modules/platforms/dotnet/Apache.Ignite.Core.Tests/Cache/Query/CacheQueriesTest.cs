...
#pragma warning restore 618
        ...
        /// <summary>
        /// Tests that <c>GetAllAsync</c> on a scan query cursor returns all entries, paging from the server
        /// without blocking the calling thread.
        /// </summary>
        [Test]
        public async Task TestScanQueryGetAllAsync()
        {
            var cache = Cache();
            await cache.RemoveAllAsync();
            await cache.PutAllAsync(Enumerable.Range(1, MaxItemCnt).ToDictionary(x => x, x => new QueryPerson(x.ToString(), x)));
            ...
            CollectionAssert.AreEquivalent(Enumerable.Range(1, MaxItemCnt), all.Select(e => e.Key));
        }

        /// <summary>
        /// Tests that <c>GetAllAsync</c> on a SQL fields query cursor returns all rows asynchronously.
        /// </summary>
        [Test]
        public async Task TestSqlFieldsQueryGetAllAsync()
        {
            var cache = Cache();
            await cache.RemoveAllAsync();
            await cache.PutAllAsync(Enumerable.Range(1, MaxItemCnt).ToDictionary(x => x, x => new QueryPerson(x.ToString(), x)));

            var cursor = cache.Query(new SqlFieldsQuery("SELECT age FROM QueryPerson ORDER BY age") { PageSize = 10 });

            var rows = await cursor.GetAllAsync();

            CollectionAssert.AreEqual(Enumerable.Range(1, MaxItemCnt), rows.Select(r => (int)r[0]));
        }

        /// <summary>
        /// Tests that <c>GetAllAsync</c> enforces the same single-consumption rules as <c>GetAll</c>: it can not
        /// be called after the cursor has already been consumed via <c>GetAll</c> or <c>GetEnumerator</c>.
        /// </summary>
        [Test]
        public async Task TestGetAllAsyncAfterConsumeThrows()
        {
            var cache = Cache();
            await cache.RemoveAllAsync();
            await cache.PutAllAsync(Enumerable.Range(1, MaxItemCnt).ToDictionary(x => x, x => new QueryPerson(x.ToString(), x)));

            var cursor = cache.Query(new ScanQuery<int, QueryPerson>());
            cursor.GetAll();
            Assert.ThrowsAsync<InvalidOperationException>(async () => await cursor.GetAllAsync());

            cursor = cache.Query(new ScanQuery<int, QueryPerson>());
            cursor.GetEnumerator();
            Assert.ThrowsAsync<InvalidOperationException>(async () => await cursor.GetAllAsync());
        }

        /// <summary>
        /// Tests that <c>GetAllAsync</c> honors a cancellation token that is already cancelled before the call.
        /// </summary>
        [Test]
        public async Task TestGetAllAsyncHonorsCancellation()
        {
            var cache = Cache();
            await cache.RemoveAllAsync();
            await cache.PutAllAsync(Enumerable.Range(1, MaxItemCnt).ToDictionary(x => x, x => new QueryPerson(x.ToString(), x)));
