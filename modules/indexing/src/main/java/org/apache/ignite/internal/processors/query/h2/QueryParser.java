...
public class QueryParser {
    ...
    private static final Pattern INTERNAL_CMD_RE = Pattern.compile(
        "^(create|drop)\\s+index|^analyze\\s|^refresh\\sstatistics|^drop\\sstatistics|^alter\\s+table|^copy|^set|^begin|" +
            "^commit|^rollback|^(create|alter|drop)\\s+user" +
            "|^kill\\s+(query|continuous|scan|compute|service|transaction|client)|show|help|grant|revoke",
