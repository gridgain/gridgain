...
public class H2RowExpireTimeIndexSelfTest extends GridCommonAbstractTest {
    ...
    public void testTreeIndexSingleRow() throws Exception {
        ...
        {
            List<List<?>> expired = cache.query(new SqlFieldsQuery("SELECT * FROM \"notEager\".Integer where id >= 42 " +
                "and id <= 42")).getAll();
        ...
        {
            List<List<?>> expired = cache.query(new SqlFieldsQuery("SELECT * FROM \"notEager\".Integer where id >= 5 " +
                "and id <= 5")).getAll();
