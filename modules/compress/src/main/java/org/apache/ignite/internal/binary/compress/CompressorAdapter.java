...
public abstract class CompressorAdapter implements Compressor {
    ...
    /** */
    @Nullable protected abstract IgniteLogger log();
    ...
    protected byte[] appraiseAndAddHeader(byte[] input, byte[] compressed, int dict) {
        ...
        if (totalSamples.incrementAndGet() % PRINT_PER == 0L && log0 != null && log0.isInfoEnabled())
            log0.info("Compression ratio: " + (float)totalCompressedSz.get() / (float)totalUncompressedSz.get() +
