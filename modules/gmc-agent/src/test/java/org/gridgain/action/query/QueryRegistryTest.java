...
public class QueryRegistryTest extends AbstractGridWithAgentTest {
    ...
    public void shouldRemoveExpiredHolders() throws Exception {
        ...
        registry.addCursor(qryId, new CursorHolder(new QueryCursorImpl<>(new ArrayList<>())));

        Thread.sleep(3000);

        assertNull(registry.findQuery(qryId));
    }
    ...
    public void shouldNotRemoveExpiredHoldersIfTheyWasFetched() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            Thread.sleep(1000);
            assertNotNull(registry.findQuery(qryId).getCursor(curId));
        }
