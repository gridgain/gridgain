...
/**
 * Ignite benchmark that performs transactional putAll + removeAll operations.
 */
public class IgnitePutAllRemoveAllTxBenchmark extends IgniteCacheAbstractBenchmark<Integer, Integer> {
    /** */
        ...
        doInTransaction(txs, args.txConcurrency(), args.txIsolation(), new Callable<Void>() {
            @Override public Void call() throws Exception {
                ...
                for (Integer key : vals.keySet()) {
                    if (keysToRemove.size() > vals.size() / 2)
                        break;
