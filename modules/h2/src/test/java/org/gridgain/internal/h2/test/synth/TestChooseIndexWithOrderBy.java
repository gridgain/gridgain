...
public class TestChooseIndexWithOrderBy {
    ...
    public static void init() throws SQLException {
        conn = DriverManager.getConnection("jdbc:gg-h2:mem:idx_order_by");
        Utils.sql(conn, "DROP TABLE IF EXISTS TEST");

        Utils.sql(conn, "CREATE TABLE TEST (ID INT PRIMARY KEY, VAL0 INT, VAL1 INT, VAL2 VARCHAR)");
        Utils.sql(conn, "CREATE INDEX IDX_VAL0 ON TEST(VAL0, ID)");
        Utils.sql(conn, "CREATE INDEX IDX_VAL0_VAL1 ON TEST(VAL0, VAL1, ID)");
