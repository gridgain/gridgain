...
import org.gridgain.internal.h2.test.trace.Player;
import org.gridgain.internal.h2.test.utils.AssertThrows;
import org.gridgain.internal.h2.tools.Backup;
import org.gridgain.internal.h2.tools.ChangeFileEncryption;
import org.gridgain.internal.h2.tools.Console;
import org.gridgain.internal.h2.tools.ConvertTraceFile;
import org.gridgain.internal.h2.tools.DeleteDbFiles;
import org.gridgain.internal.h2.tools.GUIConsole;
import org.gridgain.internal.h2.tools.Recover;
import org.gridgain.internal.h2.tools.Restore;
import org.gridgain.internal.h2.tools.RunScript;
import org.gridgain.internal.h2.tools.Script;
...
public class TestTools extends TestDb {
    ...
    private void testJdbcDriverUtils() {
        assertEquals("org.gridgain.internal.h2.Driver", JdbcUtils.getDriver("jdbc:gg-h2:~/test"));
        ...
        try {
            JdbcUtils.getConnection("org.gridgain.internal.h2.Driver", "jdbc:gg-h2x:test", "sa", "");
    ...
    private void testWrongServer() throws Exception {
        ...
        assertThrows(ErrorCode.CONNECTION_BROKEN_1, this).
                getConnection("jdbc:gg-h2:tcp://localhost:9001/test");
    ...
    private void testNonSSL() throws SQLException {
        ...
        try {
            ...
            conn = getConnection("jdbc:gg-h2:tcp://localhost:9001/mem:", "sa", "sa");
            conn.close();
    ...
    private void testSSL() throws SQLException {
        ...
        try {
            ...
            assertTrue(result.indexOf("Starts the H2 Console") < 0);
            conn = getConnection("jdbc:gg-h2:ssl://localhost:9001/mem:", "sa", "sa");
            conn.close();
            ...
            assertContains(result, "Shutting down");
            assertThrows(ErrorCode.CONNECTION_BROKEN_1, this).
            getConnection("jdbc:gg-h2:ssl://localhost:9001/mem:", "sa", "sa");
            ...
            conn = getConnection("jdbc:gg-h2:tcp://localhost:9006/mem:", "sa", "sa");
            conn.close();
            ...
            stop.shutdown();
            assertThrows(ErrorCode.CONNECTION_BROKEN_1, this).
            getConnection("jdbc:gg-h2:tcp://localhost:9006/mem:", "sa", "sa");
        } finally {
            shutdownServers();
        }
    }
    ...
    private void testConvertTraceFile() throws Exception {
        ...
        Driver.load();
        String url = "jdbc:gg-h2:" + getBaseDir() + "/toolsConvertTraceFile";
        url = getURL(url, true);
    ...
    private void testRemove() throws SQLException {
        ...
        Driver.load();
        String url = "jdbc:gg-h2:" + getBaseDir() + "/toolsRemove";
    ...
    private void testScriptRunscriptLob() throws Exception {
        Driver.load();
        String url = getURL("jdbc:gg-h2:" + getBaseDir() +
        ...
        for (int i = 0; i < 2; i++) {
            ...
            RunScript.main("-url", url, "-user", user, "-password", password,
                    "-script", fileName);
            conn = getConnection("jdbc:gg-h2:" + getBaseDir() +
    ...
    private void testScriptRunscript() throws SQLException {
        Driver.load();
        String url = getURL("jdbc:gg-h2:" + getBaseDir() + "/testScriptRunscript",
                true);
        String user = "sa", password = "abc";
        String fileName = getBaseDir() + "/b2.sql";
        ...
        conn = getConnection(
                "jdbc:gg-h2:" + getBaseDir() + "/testScriptRunscript", "sa", "abc");
    ...
    private void testBackupRestore() throws SQLException {
        Driver.load();
        String url = "jdbc:gg-h2:" + getBaseDir() + "/testBackupRestore";
        String user = "sa", password = "abc";
        ...
        Restore.main("-file", fileName, "-dir", getBaseDir(), "-db",
                "testBackupRestore", "-quiet");
        conn = getConnection("jdbc:gg-h2:" + getBaseDir() + "/testBackupRestore",
                "sa", "abc");
        ResultSet rs = conn.createStatement()
                .executeQuery("SELECT * FROM TEST");
        assertTrue(rs.next());
    ...
    private void testChangeFileEncryption(boolean split) throws SQLException {
        ...
        String url = "jdbc:gg-h2:" + dir + "/testChangeFileEncryption;CIPHER=AES";
    ...
    private void testServer() throws SQLException {
        ...
        try {
            ...
            conn = getConnection("jdbc:gg-h2:tcp://localhost:"+ port +"/test", "sa", "");
            conn.close();
            // must not be able to use a different base dir
            new AssertThrows(ErrorCode.IO_EXCEPTION_1) {
                @Override
                public void test() throws SQLException {
                    getConnection("jdbc:gg-h2:tcp://localhost:"+ port +"/../test", "sa", "");
            }};
            new AssertThrows(ErrorCode.IO_EXCEPTION_1) {
                @Override
                public void test() throws SQLException {
                    getConnection("jdbc:gg-h2:tcp://localhost:"+port+"/../test2/test", "sa", "");
            }};
            tcpServer.stop();
            ...
            new AssertThrows(ErrorCode.DATABASE_NOT_FOUND_2) {
                @Override
                public void test() throws SQLException {
                    getConnection("jdbc:gg-h2:tcp://localhost:"+prt+"/test2", "sa", "");
            }};
            new AssertThrows(ErrorCode.DATABASE_NOT_FOUND_2) {
                @Override
                public void test() throws SQLException {
                    getConnection("jdbc:gg-h2:tcp://localhost:"+prt+"/test2;ifexists=false", "sa", "");
            }};
            conn = getConnection("jdbc:gg-h2:tcp://localhost:"+prt+"/test", "sa", "");
            conn.close();
            ...
            assertThrows(ErrorCode.CONNECTION_BROKEN_1, this).
                    getConnection("jdbc:gg-h2:tcp://localhost:"+prt+"/test", "sa", "");
            ...
            conn = getConnection("jdbc:gg-h2:tcp://localhost:"+p+"/split:testSplit", "sa", "");
            conn.close();

            assertThrows(ErrorCode.IO_EXCEPTION_1, this).
                    getConnection("jdbc:gg-h2:tcp://localhost:"+p+"/../test", "sa", "");
