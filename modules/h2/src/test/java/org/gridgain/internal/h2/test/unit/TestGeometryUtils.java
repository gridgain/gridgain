...
/**
 * Tests the classes from org.gridgain.internal.h2.util.geometry package.
 */
public class TestGeometryUtils extends TestBase {
    ...
    private void testDimensionMCheckPoint(CoordinateSequence cs) {
        ...
        assertEquals(1, cs.getOrdinate(0, GeometryUtils.X));
        assertEquals(2, cs.getOrdinate(0, GeometryUtils.Y));
        assertEquals(Double.NaN, cs.getOrdinate(0, GeometryUtils.Z));
        assertEquals(3, cs.getOrdinate(0, GeometryUtils.M));
    }
    ...
    private void testDimensionZMCheckPoint(CoordinateSequence cs) {
        ...
        assertEquals(1, cs.getOrdinate(0, GeometryUtils.X));
        assertEquals(2, cs.getOrdinate(0, GeometryUtils.Y));
        assertEquals(3, cs.getOrdinate(0, GeometryUtils.Z));
        assertEquals(4, cs.getOrdinate(0, GeometryUtils.M));
    }
    ...
    private void testFiniteOnly() {
        for (int i = 0; i < NON_FINITE.length; i++) {
            testFiniteOnly(NON_FINITE[i], new EWKBUtils.EWKBTarget(new ByteArrayOutputStream(), NON_FINITE_DIMENSIONS[i]));
        }
        for (int i = 0; i < NON_FINITE.length; i++) {
            testFiniteOnly(NON_FINITE[i], new EWKTUtils.EWKTTarget(new StringBuilder(), NON_FINITE_DIMENSIONS[i]));
        }
        for (int i = 0; i < NON_FINITE.length; i++) {
            testFiniteOnly(NON_FINITE[i], new JTSUtils.GeometryTarget(NON_FINITE_DIMENSIONS[i]));
        }
    }
    ...
    private void testSRID() throws Exception {
        ...
        assertEquals(GeometryUtils.GEOMETRY_COLLECTION, vg.getTypeAndDimensionSystem());
    ...
    private void testDimensions(int expected, byte[] ewkb) {
        ...
        GeometryUtils.EnvelopeAndDimensionSystemTarget envelopeAndDimensionTarget = new GeometryUtils.EnvelopeAndDimensionSystemTarget();
