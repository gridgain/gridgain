 ...
 */
package org.gridgain.internal.h2.test.server;
...
public class TestWeb extends TestDb {
    ...
    private void testIfExists() throws Exception {
        Connection conn = getConnection("jdbc:gg-h2:mem:" + getTestName(),
                getUser(), getPassword());
        Server server = new Server();
        server.setOut(new PrintStream(new ByteArrayOutputStream()));
        ...
        try {
            ...
            client.readSessionId(result);
            result = client.get(url, "login.jsp");
            result = client.get(url, "test.do?driver=org.gridgain.internal.h2.Driver" +
                    "&url=jdbc:gg-h2:mem:" + getTestName() +
                    "&user=" + getUser() + "&password=" +
                    getPassword() + "&name=_test_");
            assertTrue(result.indexOf("Exception") < 0);
            result = client.get(url, "test.do?driver=org.gridgain.internal.h2.Driver" +
                    "&url=jdbc:gg-h2:mem:" + getTestName() + "Wrong" +
                    "&user=" + getUser() + "&password=" +
                    getPassword() + "&name=_test_");
    ...
    private void testWebApp() throws Exception {
        ...
        try {
            ...
            result = client.get(url, "test.do?driver=org.gridgain.internal.h2.Driver" +
                    ...
                    "&user=sa&password=sa&name=_test_");
            assertTrue(result.indexOf("Exception") < 0);
            result = client.get(url, "login.do?driver=org.gridgain.internal.h2.Driver" +
                    "&url=jdbc:gg-h2:mem:" + getTestName() +
                    "&user=sa&password=sa&name=_test_");
