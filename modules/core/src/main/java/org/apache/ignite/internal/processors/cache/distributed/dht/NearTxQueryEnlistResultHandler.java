...
public final class NearTxQueryEnlistResultHandler implements CI1<IgniteInternalFuture<Long>> {
    ...
    @NotNull private static GridNearTxQueryEnlistResponse createResponse(GridDhtTxQueryEnlistFuture future) {
        try {
            ...
            return new GridNearTxQueryEnlistResponse(future.cctx.cacheId(), future.cctx.dynamicDeploymentId(), future.nearFutId,
                future.nearMiniId, future.nearLockVer, future.cnt, future.tx.empty() && !future.tx.queryEnlisted(), future.newDhtNodes);
        }
        catch (IgniteCheckedException e) {
            return new GridNearTxQueryEnlistResponse(future.cctx.cacheId(), future.cctx.dynamicDeploymentId(), future.nearFutId,
                future.nearMiniId, future.nearLockVer, e);
        }
    }
    ...
    @NotNull private static GridNearTxQueryResultsEnlistResponse createResponse(GridDhtTxQueryResultsEnlistFuture fut) {
        try {
            ...
            return new GridNearTxQueryResultsEnlistResponse(fut.cctx.cacheId(), fut.cctx.dynamicDeploymentId(), fut.nearFutId,
                fut.nearMiniId, fut.nearLockVer, fut.cnt, ver, id, fut.newDhtNodes);
        }
        catch (IgniteCheckedException e) {
            return new GridNearTxQueryResultsEnlistResponse(fut.cctx.cacheId(), fut.cctx.dynamicDeploymentId(), fut.nearFutId,
                fut.nearMiniId, fut.nearLockVer, e);
        }
    }
