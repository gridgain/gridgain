 ...
 *
 * <p>For data consistency, it is important for us that the fsync delta file phase begins strictly after all page
 * replacements are completed.</p>
 *
 * <p>Usage:</p>
 * <ul>
 *     <li>{@link #block} - before you need to perform a page replacement.</li>
 *     <li>{@link #unblock} - after the page replacement has finished and written to disk. The method must be invoked
 *     even if any error occurred, so as not to hang a checkpoint.</li>
 *     <li>{@link #stopBlocking} - must be invoked before the start of the fsync phase on the checkpoint and wait for
 *     the future to complete in order to safely perform the phase.</li>
 * </ul>
 *
 * <p>Thread safe.</p>
 */
public class CheckpointPageReplacement {
    ...
    public void unblock(FullPageId pageId, @Nullable Throwable error) {
        boolean rmv = pageIds.remove(pageId);

        assert rmv : "Replacement for the page either did not start or ended: " + pageId;

        if (error != null) {
            stopBlockingFut.completeExceptionally(error);
