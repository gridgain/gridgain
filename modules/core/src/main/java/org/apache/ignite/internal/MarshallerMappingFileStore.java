...
final class MarshallerMappingFileStore {
     ...
     * @param log           Logger.
     */
    MarshallerMappingFileStore(final GridKernalContext ctx,
        ...
        mappingDir = U.resolveWorkDirectory(
            igniteWorkDir,
     ...
     */
    MarshallerMappingFileStore(final GridKernalContext ctx, final IgniteLogger log,
        final File marshallerMappingFileStoreDir) throws IgniteCheckedException {
        this.ctx = ctx;
        this.mappingDir = marshallerMappingFileStoreDir;
        this.log = log;
        ...
        try {
            ...
            while (true) {
                try (FileInputStream in = new FileInputStream(file)) {
                    ...
                    if (log.isDebugEnabled())
                        log.debug("Failed to read mapping [file=" + file.getName() + ", err=" + e + ']');
