...
public abstract class TcpCommunicationConfigInitializer extends IgniteSpiAdapter implements CommunicationSpi<Message> {
    ...
    /**
     * See {@link #setAckSendThresholdBytes(long)}.
     *
    ...
    /**
     * Sets accrued size of received messages per connection to node after which acknowledgment message is sent.
     * <p>
     * Default to {@link TcpCommunicationSpi#DFLT_ACK_SND_THRESHOLD_BYTES}.
     *
     * @param ackSndThresholdBytes Accrued size of received messages after which acknowledgment is sent.
     * @return {@code this} for chaining.
     */
    @IgniteSpiConfiguration(optional = true)
    public TcpCommunicationSpi setAckSendThresholdBytes(long ackSndThresholdBytes) {
        cfg.ackSendThresholdBytes(ackSndThresholdBytes);
