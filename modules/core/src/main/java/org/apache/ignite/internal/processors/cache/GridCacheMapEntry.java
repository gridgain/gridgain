...
public abstract class GridCacheMapEntry extends GridMetadataAwareAdapter implements GridCacheEntryEx {
    ...
    /** {@inheritDoc} */
    protected final boolean isNew() throws GridCacheEntryRemovedException {
        assert lock.isHeldByCurrentThread();
     ...
     * @throws IgniteCheckedException If failed.
     */
    protected @Nullable Object readThrough(@Nullable IgniteInternalTx tx, KeyCacheObject key, boolean reload, UUID subjId,
    ...
    private Object innerGet0(
        ...
        try {
            ...
            boolean metricsEnabled = updateMetrics && cctx.statisticsEnabled();
                    ...
                    if (expireTime > 0 && (expireTime < U.currentTimeMillis())) {
                        if (onExpired(cctx.unwrapTemporary(val), null)) {
                            ...
                            if (cctx.deferredDelete()) {
                                ...
                                if (metricsEnabled)
                                    cctx.cache().metrics0().onRead(val != null);
            ...
            if (signalEvent && cctx.events().isRecordable(EVT_CACHE_OBJECT_READ)) {
            ...
            if (retVer) {
                resVer = (isNear() && cctx.transactional()) ? ((GridNearCacheEntry)this).dhtVersion() : ver;
            ...
            if (val != null) {
                ...
                result = retVer ? entryGetResult(val, resVer, false) : val;
            }
                ...
                result = entryGetResult(null, resVer, reserve);
            }
        }
        finally {
            unlockEntry();
    ...
    /** {@inheritDoc} */
    @SuppressWarnings({"TooBroadScope"})
    @Override public final @Nullable CacheObject innerReload()
        throws IgniteCheckedException, GridCacheEntryRemovedException {
        CU.checkStore(cctx);
    ...
    @Override public final GridCacheUpdateTxResult innerSet(
        ...
        try {
            ...
            if (startVer && (intercept || lsnrCol != null))
                unswap(false);
            ...
            if (metrics && cctx.statisticsEnabled()) {
                ...
                if (entryProcRes != null && UPDATE == entryProcRes.get1())
    ...
    @Override public final GridCacheUpdateTxResult innerRemove(
        ...
        IgniteBiTuple<Boolean, Object> interceptRes;
        ...
        try {
            ...
            if (metrics && cctx.statisticsEnabled()) {
                ...
                if (entryProcRes != null && DELETE == entryProcRes.get1())
    ...
    @Override public GridTuple3<Boolean, Object, EntryProcessorResult<Object>> innerUpdateLocal(
        ...
        try {
            ...
            // Apply metrics.
            if (cctx.statisticsEnabled() && needVal)
            ...
            if (op == GridCacheOperation.TRANSFORM) {
                ...
                if (!entry.modified()) {
                    ...
                    updateMetrics(READ, transformOp, old != null);
            ...
            if (op == UPDATE) {
                ...
                if (transformCloClsName != null && cctx.events().isRecordable(EVT_CACHE_OBJECT_READ)) {
                    ...
                    cctx.events().addEvent(partition(), key, cctx.localNodeId(),null, null,
                        null, EVT_CACHE_OBJECT_READ, evtOld, evtOld != null || hadVal, evtOld,
                ...
                if (cctx.events().isRecordable(EVT_CACHE_OBJECT_PUT)) {
                    ...
                    cctx.events().addEvent(partition(), key, cctx.localNodeId(), null, null,
                        null, EVT_CACHE_OBJECT_PUT, updated, updated != null, evtOld,
                ...
                if (cctx.events().isRecordable(EVT_CACHE_OBJECT_REMOVED)) {
                    ...
                    cctx.events().addEvent(partition(), key, cctx.localNodeId(), null, null, null,
            ...
            if (res)
                updateMetrics(op, transformOp, old != null);
    ...
    @Override public GridCacheUpdateAtomicResult innerUpdate(
        ...
        try {
            ...
            GridCacheUpdateAtomicResult updateRes = c.updateResult();
            ...
            CacheObject oldVal = c.oldDataRow() != null && !c.oldRowExpiredFlag() ? c.oldDataRow().value() : null;
            CacheObject updateVal = null;
            GridCacheVersion updateVer = c.newVersion();
            ...
            if (cctx.statisticsEnabled() && INVOKE_NO_OP == updateRes.outcome() && (transformOp || updateRes.transformed()))
                cctx.cache().metrics0().onReadOnlyInvoke(oldVal != null);
            else if (cctx.statisticsEnabled() && REMOVE_NO_VAL == updateRes.outcome()
            ...
            if (op == TRANSFORM && cctx.events().isRecordable(EVT_CACHE_OBJECT_READ)) {
            ...
            if (c.cacheOperation() == UPDATE) {
                ...
                assert c.cacheOperation() == DELETE : c.cacheOperation();
            ...
            if (updateRes.success())
                updateMetrics(c.cacheOperation(), transformOp || updateRes.transformed(), oldVal != null);
            ...
            if (lsnrs != null) {
                ...
                cctx.continuousQueries().onEntryUpdated(lsnrs,
                    ...
                    primary,
                    false,
                    c.updateResult().updateCounter(),
                    fut,
                    topVer);
            }

            if (intercept && c.wasIntercepted()) {
                assert c.cacheOperation() == UPDATE || c.cacheOperation() == DELETE : c.cacheOperation();

                Cache.Entry<?,?> entry = new CacheLazyEntry<>(
                    cctx,
                    key,
                    null,
                    c.cacheOperation() == UPDATE ? updateVal : oldVal,
                    null,
                    keepBinary,
                    c.updateResult().updateCounter()
                );
        ...
        onUpdateFinished(c.updateResult().updateCounter());
     ...
     */
    static IgniteBiTuple<Long, Long> initialTtlAndExpireTime(IgniteCacheExpiryPolicy expiry) {
        assert expiry != null;
     ...
     * @return Result.
     */
    GridTuple3<Long, Long, Boolean> ttlAndExpireTime(IgniteCacheExpiryPolicy expiry, long ttl, long expireTime) {
        assert !obsolete();
     ...
     */
    private void updateTtl(ExpiryPolicy expiryPlc) throws IgniteCheckedException {
            ...
            try {
                ...
                if (checkExpired()) {
                    if (cctx.deferredDelete()) {
                        ...
                        rmv = markObsolete0(cctx.versions().next(ver), true, null);
            ...
            IgnitePredicate<CacheDataRow> p = new IgnitePredicate<CacheDataRow>() {
                @Override public boolean apply(@Nullable CacheDataRow row) {
                    ...
                    if (cctx.group().persistenceEnabled()) {
                        if (!isStartVer) {
                            update0 = cctx.atomic() ? ATOMIC_VER_COMPARATOR.compare(currentVer, ver) < 0 :
                                currentVer.compareTo(ver) < 0;
                        }
                        else
                            update0 = true;
                    }
    ...
    private boolean onExpired(CacheObject expiredVal, GridCacheVersion obsoleteVer) throws IgniteCheckedException {
        return onExpired(expiredVal, obsoleteVer, false);
    }
     ...
     * @throws IgniteCheckedException If failed.
     */
    private boolean onExpired(CacheObject expiredVal, GridCacheVersion obsoleteVer, boolean remove) throws IgniteCheckedException {
...
//        }
    ...
    @Override public <K, V> Cache.Entry<K, V> wrap() {
        try {
            ...
            return new CacheEntryImpl<>(key.value(cctx.cacheObjectContext(), false),
                CU.value(val, cctx, false), ver);
        }
    ...
    @Override public <K, V> CacheEntryImplEx<K, V> wrapVersioned() {
        ...
        try {
            return new CacheEntryImplEx<>(key.value(cctx.cacheObjectContext(), false), null, ver);
        }
        finally {
            unlockEntry();
        }
    ...
    private <K, V> CacheEntryImplEx<K, V> wrapVersionedWithValue() {
        ...
        try {
            V val = this.val == null ? null : this.val.value(cctx.cacheObjectContext(), false);

            return new CacheEntryImplEx<>(key.value(cctx.cacheObjectContext(), false), val, ver);
        }
        finally {
            unlockEntry();
        }
    ...
    public final @Nullable List<GridCacheMvccCandidate> mvccAllLocal() {
        lockEntry();
     ...
     */
    private void updateMetrics(GridCacheOperation op, boolean transformed, boolean hasOldVal) {
        if (cctx.statisticsEnabled()) {
            if (op == DELETE) {
                cctx.cache().metrics0().onRemove();
    ...
    @Override public GridCacheUpdateTxResult mvccUpdateRowsWithPreloadInfo(
        ...
        try {
            ...
            List<DataEntry> walEntries = walEnabled ? new ArrayList<>(entries.size() + 1) : Collections.emptyList();
