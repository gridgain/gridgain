...
public class BinaryObjectImpl extends BinaryObjectExImpl implements Externalizable, KeyCacheObject {
    /** */
    ...
    /** */
    private KeyCacheObject tryCompress(EntryCompressionStrategy compressionStrategy) {
        if (compressionStrategy == null)
            return this;

        byte[] tryCompressed = compressionStrategy.tryCompress(arr);

        return tryCompressed == null ? new UncompressedForm(ctx, arr, start, part, compressionStrategy) :
            new CompressedForm(ctx, arr, start, part, compressionStrategy, tryCompressed);
    }
    ...
    /** Entry compression: this binary object is not compressable for the current cache. */
    private static class UncompressedForm extends BinaryObjectImpl {
        ...
        public UncompressedForm() {
            throw new IgniteException();
        }

        /** */
        public UncompressedForm(BinaryContext ctx, byte[] arr, int start, int part,
            EntryCompressionStrategy compressionStrategy) {
            super(ctx, arr, start);
        ...
        @Override public KeyCacheObject prepareForCache(CacheObjectContext ctx, boolean compress) {
            EntryCompressionStrategy otherStrategy = ctx.compressionStrategy();

            if (!compressionStrategy.equals(otherStrategy))
                throw new IgniteException("Should not re-prepare compressed binary object form for a different cache.");

            return this;
        }
    }

    /** Entry compression: this binary object is compressed for the current cache. */
    private static class CompressedForm extends UncompressedForm {
        /** Should never be serialized. */
        private static final long serialVersionUID = System.currentTimeMillis();

        /** Compressed form of {@link #arr}, ready to be written to page or WAL. Local to a node and cache group. */
        ...
        /** */
        public CompressedForm(BinaryContext ctx, byte[] arr, int start, int part,
            EntryCompressionStrategy compressionStrategy, byte[] compressedArr) {
            super(ctx, arr, start, part, compressionStrategy);

            this.compressedArr = compressedArr;
        }

        /** {@inheritDoc} */
        @Override public int putValue(long addr) throws IgniteCheckedException {
            return CacheObjectAdapter.putValue(addr, CacheObject.TYPE_BINARY_COMPRESSED, compressedArr,
                0, compressedArr.length);
        }

        /** {@inheritDoc} */
        @Override public boolean putValue(final ByteBuffer buf, int off, int len) throws IgniteCheckedException {
            return CacheObjectAdapter.putValue(CacheObject.TYPE_BINARY_COMPRESSED, buf, off, len, compressedArr, 0);
        }
