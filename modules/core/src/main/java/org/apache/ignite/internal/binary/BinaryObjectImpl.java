...
public class BinaryObjectImpl extends BinaryObjectExImpl implements Externalizable, KeyCacheObject {
    ...
    private KeyCacheObject tryCompress(EntryCompressionStrategy compressionStrategy) {
        ...
        return new CompressedForCache(ctx, arr, start, part,
            tryCompressed == null ? UNCOMPRESSED : tryCompressed, compressionStrategy);
    }
    ...
    /** */
    private static class CompressedForCache extends BinaryObjectImpl {
        ...
        public CompressedForCache() {
            throw new IgniteException();
        }

        /** */
        public CompressedForCache(BinaryContext ctx, byte[] arr, int start, int part, byte[] compressedArr,
        ...
        /** {@inheritDoc} */
        @Override public int putValue(long addr) throws IgniteCheckedException {
            if (compressedArr == UNCOMPRESSED)
                return super.putValue(addr);
        ...
        @Override public boolean putValue(final ByteBuffer buf, int off, int len) throws IgniteCheckedException {
            if (compressedArr == UNCOMPRESSED)
                return super.putValue(buf, off, len);
        ...
        @Override public int valueBytesLength(CacheObjectContext ctx) throws IgniteCheckedException {
            if (compressedArr == UNCOMPRESSED)
                return super.valueBytesLength(ctx);
        ...
        @Override public KeyCacheObject prepareForCache(CacheObjectContext ctx, boolean compress) {
            ...
            if (!compressionStrategy.equals(otherStrategy))
                throw new IgniteException("Should not re-prepare compressed binary object form");
