...
public class BlockedTombstoneClearingTest extends GridCommonAbstractTest {
    ...
    public void testMovingToEvicted() throws Exception {
        ...
        loadDataToPartition(testPart, getTestIgniteInstanceName(0), DEFAULT_CACHE_NAME, cnt, 0, 3);

        forceCheckpoint();

        stopGrid(1);

        loadDataToPartition(testPart, getTestIgniteInstanceName(0), DEFAULT_CACHE_NAME, delta, cnt, 3);

        // Removal required for triggering full rebalancing to satisfy heuristic.
        List<Integer> clearKeys = partitionKeys(grid(0).cache(DEFAULT_CACHE_NAME), testPart, rmv, cnt);
        ...
        GridDhtLocalPartition evicting = g2.cachex(DEFAULT_CACHE_NAME).context().topology().localPartition(testPart);
