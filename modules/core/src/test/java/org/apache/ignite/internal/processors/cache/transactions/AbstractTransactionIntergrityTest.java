...
public class AbstractTransactionIntergrityTest extends GridCommonAbstractTest {
    ...
    /**
     * Calculates total amount of coins for every thread for every node and checks that coins difference is zero (transaction
     * integrity is saved).
     */
            ...
            for (int j = 0; j < txThreadsCount(); j++) {
                ...
                try (Transaction tx = node.transactions().txStart(PESSIMISTIC, REPEATABLE_READ)) {
                    for (int i = 0; i < accountsCount(); i++) {
                        ...
                        Assert.assertNotNull("Account state has lost [node=" + node.name() + ", cache=" + cacheName +
                            ", accNo=" + i + "]", state);
                ...
                if (initAmount[j] != totalCoins.size()) {
                    ...
                    log.error(String.format("Total amount of coins before and after transfers are not same. " +
                            "Lost coins: %s. Duplicate coins: %s.",
    ...
    static class TxState {
         ...
         */
        public TxState(AccountState before1, AccountState before2, AccountState after1, AccountState after2,
            Set<Integer> transferredCoins) {
