...
//        processSize2Test(GridDeploymentMode.SHARED);
    ...
    public void testCheckTaskClassloaderCache(DeploymentMode depMode) throws Exception {
        ...
        client.compute().execute(taskCls1.newInstance(), server1.localNode().id());
        client.compute().execute(taskCls2.newInstance(), server1.localNode().id());
        client.compute().execute(taskCls1.newInstance(), server1.localNode().id());
