            ...
            switch (crd.cluster().state()) {
                case INACTIVE:
                    assertNotNull(assertThrows(
                        ...
                        "Can not perform the operation because the cluster is inactive. Note, that the cluster is " +
                            "considered inactive by default if Ignite Persistent Store is used to let all the nodes join " +
                            "the cluster. To activate the cluster call Ignite.active(true)."
                    ));
