...
public class BPlusTreeReplaceTest extends GridCommonAbstractTest {
    /** */
    ...
    public void testConcurrentPutRemove() throws Exception {
        for (int i = 0; i < 50; i++) {
            ...
            if (k == 1) {
                ...
                IgniteInternalFuture<?> putFut0 = GridTestUtils.runAsync(() -> {
                    try {
                        barrier.await();

                        BPlusTree.waitRemoveX = true;

                        Pair row = new Pair(5, 999);
                ...
                // This is the replace operation.
                IgniteInternalFuture<?> putFut1 = GridTestUtils.runAsync(() -> {
                    try {
                        barrier.await();

                        BPlusTree.waitPutXFut.get();

                        BPlusTree.waitRemoveX = false;

                        Pair row = new Pair(3, 0);

                        tree.putx(row);

                        BPlusTree.waitRemoveXFut.onDone();
                    }
                    catch (Exception e) {
                        throw new IgniteException(e);
                    }
            ...
            try {
                putFut.get(1, TimeUnit.HOURS);
            }
            finally {
                remFut.get(1, TimeUnit.HOURS);
            }
