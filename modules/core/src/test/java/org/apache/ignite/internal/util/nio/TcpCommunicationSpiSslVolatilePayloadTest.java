...
public class TcpCommunicationSpiSslVolatilePayloadTest extends GridAbstractCommunicationSelfTest<CommunicationSpi<Message>> {
    ...
    public void test() throws Exception {
        ...
        try {
            assertTrue(waitForCondition(() -> msgCreatedCntr.get() == msgReceivedCntr.get(), 60_000));
        }
        finally {
            System.out.println("Created=" + msgCreatedCntr.get() + ", received=" + msgReceivedCntr.get() + ", map=" + messages);
        }
    }
    ...
    private static class TestCommunicationListener implements CommunicationListener<Message> {
        ...
        @Override public void onMessage(UUID nodeId, Message msg, IgniteRunnable msgC) {
            ...
            if (msg instanceof TestVolatilePayloadMessage) {
                ...
                TestVolatilePayloadMessage expMsg = messages.remove(testMsg.index());

                assertNotNull("Duplicated message received [index=" + testMsg.index() + "]", expMsg);
