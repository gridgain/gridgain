...
public abstract class IgniteLockAbstractSelfTest extends IgniteAtomicsAbstractTest
    ...
    public void testFailover() throws Exception {
        assumeTrue(atomicsCacheMode() != LOCAL);
    ...
    public void testFairLockIsAbleToBeAcquiredAfterRelease() throws Exception {
        ...
        List<IgniteLock> locks = IntStream.range(0, gridCount())
            .mapToObj(i -> grid(i).reentrantLock("test", true, true, true))
            .collect(Collectors.toList());

        CountDownLatch lockEnterLatch = new CountDownLatch(gridCount() - 1);
        locks.get(0).lock();
        try {
            acquireLockInSeparateThreads(locks.subList(1, gridCount()), lockEnterLatch);
        }
        finally {
            locks.get(0).unlock();
        }
