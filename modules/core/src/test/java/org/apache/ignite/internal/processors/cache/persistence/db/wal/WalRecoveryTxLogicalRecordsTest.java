...
public class WalRecoveryTxLogicalRecordsTest extends GridCommonAbstractTest {
    ...
    private DataEntry entryOp(GridCacheSharedContext cctx, int key, int cacheId, int partId, GridCacheOperation op,
        ...
        return new DataEntry(cacheId, key0, val0, op, cctx.versions().next(cacheCtx.topology().readyTopologyVersion()),
            cctx.versions().next(cacheCtx.topology().readyTopologyVersion()), 0, partId, cntr);
    }
